...
1. Download the latest firmware image from the most recent [Release](https://github.com/ammolytics/open-trickler-peripheral/releases).  
  Choose a firmware image that matches your device (e.g. `pi0w_prod.firmware.zip` is for a Pi Zero W.  
  Images labeled `dev` provide ssh access and `prod` images do not.  
  Note: These files are generated by an [automated build](https://github.com/ammolytics/opentrickler-buildroot/actions/workflows/build-ot-firmware.yml) process and are copied to this repo.
1. Flash your microSD card using [Raspberry Pi Imager](https://www.raspberrypi.com/software/) or [balenaEtcher](https://www.balena.io/etcher/).
...
1. Turn on your scale.  
  **Note:** The mobile app will not connect until the Pi detects a working scale.
